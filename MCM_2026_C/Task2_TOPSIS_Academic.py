...
class BiasAnalyzer:
    ...
    def compute_effective_weights(contestants: list) -> dict:
        ...
        百分比法: w_pct^J = Var(P^J) / (Var(P^J) + Var(P^F))
        """
        P_J = np.array([c['P_J'] for c in contestants])
        P_F = np.array([c['P_F'] for c in contestants])
        
        var_J = np.var(P_J)
        var_F = np.var(P_F)
        
        # 百分比法有效权重
        total_var = var_J + var_F
        if total_var > 0:
            w_pct_J = var_J / total_var
            w_pct_F = var_F / total_var
        else:
            w_pct_J = w_pct_F = 0.5
        ...
        return {
            ...
            'rank_fan_weight': 0.5,
            'pct_judge_weight': w_pct_J,
            'pct_fan_weight': w_pct_F,
            'var_judge': var_J,
            'var_fan': var_F
        }
...
class TOPSISRecommender:
    ...
    def evaluate_mechanisms(analysis_results: list) -> dict:
        ...
        for result in analysis_results:
            ...
            if weights:
                pct_gini = abs(weights.get('pct_judge_weight', 0.5) - 0.5) * 2
                pct_stats['fair_scores'].append(1 - pct_gini)
                rank_stats['fair_scores'].append(1)  # 排名法固定等权，完全公平
